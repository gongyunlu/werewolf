...
export async function runDay(input: DayInput): Promise<DayResult> {
  ...
  const settled = await settleBadgeAfterDeaths(input.state, actions);
  observe?.(settled);
  const blast = await runBlastWindow(settled, 'day', actions, observe, onFlow);
  if (blast.blasted) {
    return { state: blast.state, speeches: [], exiledId: null, ballots: [] };
  }
  ...
  const exile = await runExile(state, actions, speechOrder, onBallot, onFlow, observe);
  if (!exile.aborted)
  ...
  return {
    state: exile.state,
    speeches: [...speeches, ...exile.speeches],
