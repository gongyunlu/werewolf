...
export async function speakInOrder(
  ...
  function playerAt(seatNo: number): PlayerState {
    const player = playerBySeatNo.get(seatNo);
    if (!player) throw new Error(`发言顺序里的座位号不在局内：${seatNo}`);
    return player;
  }

  // 顺序整份先换成玩家 id：轮到谁时都得知道自己后面还有谁，所以每次提问都把它带下去。
  const order = seatNos.map((seatNo) => playerAt(seatNo).id);

  const speeches: Speech[] = [];
  for (const seatNo of seatNos) {
    const player = playerAt(seatNo);
    speeches.push({
      turn,
      playerId: player.id,
      seatNo,
      content: await actions.speak(turn, player.id, order),
    });
  }
