...
export async function runExile(
  ...
  await onFlow?.(state, { key: 'exile-vote', text: '发言结束，请所有存活玩家同时进行放逐投票。' });
  ...
  const first: Ballot = { round: 'exile', casts: ballot, outcome };
  ...
  const second: Ballot = { round: 'exile_pk', casts: pkBallot, outcome: pkOutcome };
  await onBallot?.(second);
